...
export default function Login({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userFound) {
        setStatus({ text: "เข้าสู่ระบบสำเร็จ!", type: "success" });
      } else {
        setStatus({
          text: "ไม่พบ Gmail นี้ในระบบ กรุณาติดต่อผู้ดูแลระบบ",
          type: "error",
        });
      }
    } catch (error) {
        ...
        <div className="glass-panel rounded-3xl p-8 md:p-10 relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <div className="relative group">
                <input
                  ...
                  className="w-full px-5 py-4 pl-12 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-red-500 focus:border-red-500 focus:bg-white transition-all duration-200 outline-none text-gray-800 placeholder-gray-400"
                />
                <svg
                  className="w-5 h-5 text-gray-400 absolute left-4 top-4.5 group-focus-within:text-red-500 transition-colors duration-200"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-700 text-white font-bold py-4 px-6 rounded-2xl transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-red-500/30 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
