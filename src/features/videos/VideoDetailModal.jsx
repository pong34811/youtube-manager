      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-center">
            <p className="text-2xl  text-[var(--text-primary)]">{parseInt(v.viewCount || 0).toLocaleString()}</p>
            <p className="text-xs text-[var(--text-secondary)]">{t("common.views")}</p>
          </div>
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-center">
            <p className="text-2xl  text-[var(--text-primary)]">{parseInt(v.likeCount || 0).toLocaleString()}</p>
            <p className="text-xs text-[var(--text-secondary)]">{t("common.likes")}</p>
          </div>
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-center">
            <p className="text-2xl  text-[var(--text-primary)]">{parseInt(v.commentCount || 0).toLocaleString()}</p>
            <p className="text-xs text-[var(--text-secondary)]">{t("common.comments")}</p>
          </div>
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-center">
            <p className="text-2xl  text-[var(--text-primary)]">{new Date(video.snippet.publishedAt).toLocaleDateString("th-TH")}</p>
