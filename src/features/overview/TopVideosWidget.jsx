...
export default function TopVideosWidget({ videos }) {
  ...
  return (
    <Card>
      <h3 className="text-base  text-[var(--text-primary)] mb-4">{t("overview.topVideos")}</h3>
      <div className="space-y-3">
        {top.map((v, i) => (
          <div key={v.id} className="flex items-center space-x-3">
            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs  ${i < 3 ? "bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300" : "bg-gray-100 dark:bg-gray-700 text-gray-500"}`}>
              {i + 1}
            </span>
